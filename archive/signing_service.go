...
func (s *SigningService) CreateAndExecuteSigning(
	...
	keygenSession, err := KeygenServiceInstance.GetSession(keygenSessionID)
	if err != nil {
		s.mu.Unlock()
		...
		for signerID, sigParty := range session.sigParties {
			...
			case sigData := <-sigParty.EndCh:
				...
				if session.Signature == nil {
					...
					copy(sigS[32-len(sBytes):], sBytes)
...
// SigningServiceInstance is the global signing service instance
var SigningServiceInstance = NewSigningService()
